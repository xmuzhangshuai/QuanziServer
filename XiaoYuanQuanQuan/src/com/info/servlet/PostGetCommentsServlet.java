...
@WebServlet(name="getCommentsServlet",urlPatterns="/post/getCommentList")
public class PostGetCommentsServlet extends HttpServlet{
	/**
	 * 
	 */
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(pageNow<0){
			...
			int p_postid = Integer.parseInt(request.getParameter(PostTable.P_POSTID));
			int p_userid = Integer.parseInt(request.getParameter(PostTable.P_USERID));
			int userid = Integer.parseInt(request.getParameter(UserTable.U_ID));
			
			HashMap<String,Object> result = pImpl.getPostComments(p_postid, p_userid, pageNow, userid);
